...
namespace WarCroft.Core
{
    public class WarController
    {
        ...
        public string GetStats()
        {
            ...
            var characters = characterParty.OrderByDescending(c => c.IsAlive).ThenByDescending(c => c.Health);
            foreach (var items in characters)
            {
                sb.AppendLine(string.Format(SuccessMessages.CharacterStats, items.Name, items.Health, items.BaseHealth, items.Armor, items.BaseArmor, items.IsAlive ? "Alive" : "Dead"));
            }
        ...
        public string Attack(string[] args)
        {
            ...
            Character reciever = characterParty.FirstOrDefault(c => c.Name == args[1]);
            if (attacker == null)
            {
